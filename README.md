...
## 4-1. SDK 설계의 3가지 고민: 사용성, 쉬운 연동, 보안

웰컴토스트의 핵심 아키텍처는 세 가지 주요 컴포넌트로 구성됩니다:

1. **클라이언트 웹사이트**: 사용자에게 튜토리얼을 제공하는 대상 사이트
2. **에디터 인터페이스**: 관리자가 튜토리얼을 시각적으로 구성하는 GUI
3. **중앙 데이터 저장소**: Supabase를 활용한 데이터 관리 레이어

데이터 흐름은 다음과 같습니다:
- 웹사이트는 고유 API 키를 포함한 스크립트를 삽입하여 시스템과 연결
- 튜토리얼 데이터는 Supabase를 통해 REST API 방식으로 교환
- Row Level Security 정책을 구성하여 API 키 기반 인증을 구현, 각 클라이언트가 자신의 데이터만 접근하도록 제한

에디터에서는 대상 웹사이트를 iframe으로 로드하여 실시간 프리뷰와 요소 선택이 가능하게 했습니다. CORS 관련 보안 문제를 해결하기 위해 postMessage API를 사용했으며, 공식 문서의 권장사항에 따라 target origin을 명확히 지정하여 보안 취약점을 방지했습니다.
